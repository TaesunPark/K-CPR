(function () {
   ...
   var nextTime = 0;
   var k=0;
   var lastTwoTimes=[new Date(), new Date()];
...
//   console.log(position);
   ...
   if(findPeakSVM(SVMdataList) > 0){
	   ...
	   //현재 시간 추가
	   setRecentTime(lastTwoTimes);
	   
	   if(getPeakGap(lastTwoTimes) > 200)num++;
	   
	   if(getPeakGap(lastTwoTimes) > 650){
   ...
   if(new Date() - lastTwoTimes[1] > 3000){
	   document.getElementById('ui-page-id').style.backgroundColor= "#31dc70";
       document.getElementById('phrase').innerHTML = "Stopped";
   }
}


function setRecentTime(lastTwoTimes){
	lastTwoTimes.push(new Date());
	lastTwoTimes.shift();
}

function getPeakGap(lastTwoTimes){
	return lastTwoTimes[1] - lastTwoTimes[0]
}
...
var SVMdataList = [];
var peakList = [];
const SVMdataListSize = 3; //홀수 넣어주기
const peakListSize = 4;
